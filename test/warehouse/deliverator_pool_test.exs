defmodule Warehouse.DeliveratorPoolTest do
  use ExUnit.Case
  alias Warehouse.{DeliveratorPool}
  doctest Warehouse.DeliveratorPool
  ...
  describe "#available_deliverator" do
    ...
    test "fetches idle deliverator" do
      {:ok, first_pid} = DeliveratorPool.available_deliverator
      {:ok, second_pid} = DeliveratorPool.available_deliverator
      state = :sys.get_state(DeliveratorPool)
    ...
    test "return error when maxed out" do
      state = :sys.get_state(DeliveratorPool)
  ...
  describe "#flag_deliverator_busy" do
    test "flags deliverator to busy" do
      {:ok, pid} = DeliveratorPool.available_deliverator
      DeliveratorPool.flag_deliverator_busy(pid)
      state = :sys.get_state(DeliveratorPool)
  ...
  describe "#flag_deliverator_idle" do
    test "flags deliverator to idle" do
      {:ok, pid} = DeliveratorPool.available_deliverator
